...
function readerAccountUpgrade(PDO $db): void
{
    $db->exec("CREATE TABLE IF NOT EXISTS wts_members (id BIGINT UNSIGNED AUTO_INCREMENT PRIMARY KEY,display_name VARCHAR(120) NOT NULL,profile_slug VARCHAR(80) NOT NULL UNIQUE,email VARCHAR(190) NOT NULL UNIQUE,password_hash VARCHAR(255) NOT NULL,email_confirmed_at TIMESTAMP NULL DEFAULT NULL,confirmation_token_hash CHAR(64) NULL,confirmation_sent_at TIMESTAMP NULL DEFAULT NULL,bio VARCHAR(500) NULL,created_at TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP,updated_at TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,INDEX idx_wts_members_confirmation (confirmation_token_hash)) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci");
    $db->exec("CREATE TABLE IF NOT EXISTS wts_purchase_intents (id BIGINT UNSIGNED AUTO_INCREMENT PRIMARY KEY,member_id BIGINT UNSIGNED NOT NULL,product_id BIGINT UNSIGNED NULL,product_name VARCHAR(255) NOT NULL,amount DECIMAL(10,2) NOT NULL,currency CHAR(3) NOT NULL,status ENUM('pending','paid','cancelled','refunded') NOT NULL DEFAULT 'pending',reference_token CHAR(40) NOT NULL UNIQUE,paypal_transaction_id VARCHAR(190) NULL UNIQUE,created_at TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP,updated_at TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,INDEX idx_wts_purchase_member (member_id,created_at)) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci");
    if (databaseTableExists('wts_blog_comments')) { $columns=$db->query('SHOW COLUMNS FROM wts_blog_comments')->fetchAll(PDO::FETCH_COLUMN); if (!in_array('member_id',$columns,true)) $db->exec('ALTER TABLE wts_blog_comments ADD COLUMN member_id BIGINT UNSIGNED NULL AFTER parent_id, ADD INDEX idx_wts_comments_member (member_id)'); }
}
...
function databaseUpdates(): array
{
    return [
      ...
      '2026-08-product-catalog'=>['Products and PayPal giving','Creates the custom product catalog with fixed prices, suggested giving amounts, and PayPal handoff settings.', fn(PDO $db)=>$db->exec("CREATE TABLE IF NOT EXISTS wts_products (id BIGINT UNSIGNED AUTO_INCREMENT PRIMARY KEY,slug VARCHAR(190) NOT NULL UNIQUE,name VARCHAR(255) NOT NULL,short_description VARCHAR(500) NULL,description TEXT NULL,image_url VARCHAR(1000) NULL,badge VARCHAR(80) NULL,pricing_mode ENUM('fixed','contribution') NOT NULL DEFAULT 'fixed',price DECIMAL(10,2) NOT NULL DEFAULT 0.00,suggested_amounts VARCHAR(255) NULL,minimum_amount DECIMAL(10,2) NOT NULL DEFAULT 1.00,maximum_amount DECIMAL(10,2) NULL,allow_custom_amount TINYINT(1) NOT NULL DEFAULT 1,button_label VARCHAR(120) NOT NULL DEFAULT 'Continue with PayPal',fulfillment_note VARCHAR(500) NULL,display_order SMALLINT UNSIGNED NOT NULL DEFAULT 10,status ENUM('draft','published','archived') NOT NULL DEFAULT 'draft',created_at TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP,updated_at TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,INDEX idx_wts_products_status_order (status,display_order)) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci")],
      '2026-08-reader-accounts'=>['Reader accounts and purchase history','Adds confirmed reader accounts, public profiles, linked comments, and PayPal checkout activity.', fn(PDO $db)=>readerAccountUpgrade($db)],
    ];
}
