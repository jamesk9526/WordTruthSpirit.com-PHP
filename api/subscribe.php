...
if ($db) try {
    ...
    } else { $statement=$db->prepare('INSERT INTO wts_subscribers (email,status,token) VALUES (:email,"pending",:token) ON DUPLICATE KEY UPDATE status="pending",token=VALUES(token),updated_at=CURRENT_TIMESTAMP');$statement->execute(['email'=>$email,'token'=>hash('sha256',$confirmationToken)]);$id=(string)$db->lastInsertId();if($id==='0'){ $lookup=$db->prepare('SELECT id FROM wts_subscribers WHERE email=?');$lookup->execute([$email]);$id=(string)$lookup->fetchColumn(); }$status='pending'; }
    if($status==='pending'){
