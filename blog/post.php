...
function publicPostInitial(string $value): string
{
    $initial = function_exists('mb_substr') ? mb_substr($value, 0, 1) : substr($value, 0, 1);
...
<?php else: ?>
  ...
  <article class="author-article" data-post-engagement data-post-slug="<?=e((string)$post['slug'])?>" data-engagement-url="<?=url('api/post-engagement.php')?>">
    <header class="author-post-header"><p class="kicker"><?= e($post['category']) ?> · Reflection</p><h1><?= e($post['title']) ?></h1><p class="post-deck"><?= e($post['excerpt']) ?></p><div class="author-byline"><span class="author-initial"><?= e(publicPostInitial((string)$post['author'])) ?></span><p><strong><?= e($post['author']) ?></strong><small><?= date('F j, Y', strtotime($post['published_at'])) ?> · <?= (int) $post['reading_minutes'] ?> min read</small></p></div></header>
