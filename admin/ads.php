...
<div class="ads-admin-layout">
  <section class="admin-panel ads-list">
    ...
    <?php foreach ($ads as $ad): $adId=(string)($ad['id']??''); $status=strtolower(adStatusLabel($ad)); $isSelected=$editingId!==''&&$editingId!=='new'&&$editingId===$adId; ?>
      <article class="<?=$isSelected?'selected':''?>" data-ad-row data-ad-placement="<?=e(adEditorValue($ad,'placement'))?>" data-ad-status="<?=e($status)?>" data-ad-search="<?=e(function_exists('mb_strtolower') ? mb_strtolower(adEditorValue($ad,'name').' '.adEditorValue($ad,'title').' '.adEditorValue($ad,'sponsor')) : strtolower(adEditorValue($ad,'name').' '.adEditorValue($ad,'title').' '.adEditorValue($ad,'sponsor')))?>">
