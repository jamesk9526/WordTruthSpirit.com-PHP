...
<head>
  ...
  <link rel="stylesheet" href="<?= url('assets/styles.css?v=20260815a') ?>">
</head>
...
<div class="admin-shell<?= $dedicatedAdminEditor ? ' editor-workspace-shell' : '' ?>">
  <?php if (adminLoggedIn() && !$dedicatedAdminEditor): ?><header class="admin-topbar"><button type="button" class="admin-menu-button" aria-controls="admin-sidebar" aria-expanded="false">Menu</button><img class="admin-topbar-sprite" src="<?=url('assets/images/spirit-dove.png')?>" alt="" aria-hidden="true"><span><?= e($adminTitle) ?></span><strong><?= e($_SESSION['wts_admin_name'] ?? 'Administrator') ?></strong></header><?php endif; ?>
