...
if($_SERVER['REQUEST_METHOD']==='POST'){
  ...
  elseif(!setAppSetting('site.experience',json_encode($experience,JSON_UNESCAPED_SLASHES|JSON_UNESCAPED_UNICODE))||!setAppSetting('ministry.donationUrl',trim((string)($_POST['donation_url']??'')))||!setAppSetting('ministry.donationLabel',trim((string)($_POST['donation_label']??'Support the ministry')))||!setAppSetting('seo.googleVerification',trim((string)($_POST['google_verification']??'')))||!setAppSetting('seo.searchConsoleProperty',trim((string)($_POST['search_console_property']??''))))$error='Could not save settings. Confirm the database is connected.';
  else{
...
<section class="admin-panel settings-section">
  ...
  <p>Choose where readers can subscribe and keep the invitation consistent across the journal, article pages, footer, and dismissible bottom-page banner.</p>
  ...
  <div class="placement-options">
    <label class="toggle-label"><input type="checkbox" name="subscription_blog_panel" <?=$experience['subscription']['placements']['blogPanel']?'checked':''?>> Journal signup panel</label>
