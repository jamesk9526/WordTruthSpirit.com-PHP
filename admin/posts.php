...
$db=database();if($_SERVER['REQUEST_METHOD']==='POST'){verifyCsrf();$db->prepare('DELETE FROM wts_posts WHERE id=?')->execute([(int)$_POST['id']]);header('Location:'.url('admin/posts.php?deleted=1'));exit;}
$posts=$db->query('SELECT id,title,slug,category,status,published_at FROM wts_posts ORDER BY published_at DESC,created_at DESC')->fetchAll();$adminTitle='Journal';require __DIR__.'/_header.php';
?>
