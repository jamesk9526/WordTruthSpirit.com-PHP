...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!$values['title'] || !preg_match('/^[a-z0-9-]+$/', $values['slug']) || !$values['body']) {
        ...
        try {
            if ($legacy) {
                $legacyValues = ['title'=>$values['title'],'slug'=>$values['slug'],'category'=>$values['category'],'excerpt'=>$values['excerpt'],'content'=>$values['body'],'author'=>$values['author'],'read_time'=>$values['reading_minutes'].' min read','published'=>$values['status'] === 'published' ? 1 : 0,'date'=>$values['published_at'] ? date('Y-m-d', strtotime($values['published_at'])) : null,'tags'=>$values['tags'],'cover_image'=>$values['cover_image'],'meta_title'=>$values['meta_title'],'meta_description'=>$values['meta_description'],'featured'=>$values['featured']?1:0];
