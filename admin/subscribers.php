...
$rows = $legacy
    ...
    : $db->query('SELECT email,status,NULL AS source,NULL AS confirmed_at,created_at FROM wts_subscribers ORDER BY created_at DESC')->fetchAll();
...
<section class="admin-panel"><div class="admin-table-wrap"><table class="admin-table"><thead><tr><th>Email</th><th>Status</th><th>Source</th><th>Joined</th></tr></thead><tbody><?php foreach ($rows as $row): ?><tr><td><strong><?= e($row['email']) ?></strong></td><td><span class="status-pill"><?= e($row['status']) ?></span></td><td><?= e($row['source'] ?: 'Website') ?></td><td><?= date('M j, Y', strtotime($row['created_at'])) ?></td></tr><?php endforeach; ?></tbody></table></div><?php if (!$rows): ?><p>No subscribers yet.</p><?php endif; ?></section>
